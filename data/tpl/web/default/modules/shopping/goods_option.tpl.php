...
<script language="javascript">
            ...
            $("#add-spec").html("正在处理...").attr("disabled", "true").toggleClass("btn-primary");
                    var url = "<?php  echo create_url('site/module',array('do'=>'spec','name'=>'shopping'))?>";
                    $.ajax({
                    "url": url,
    ...
    $("#add-specitem-" + specid).html("正在处理...").attr("disabled", "true");
            var url = "<?php  echo create_url('site/module',array('do'=>'specitem','name'=>'shopping'))?>" + "&specid=" + specid;
            $.ajax({
            "url": url,
                    success:function(data){
