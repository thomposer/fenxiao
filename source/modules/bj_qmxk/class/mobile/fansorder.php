...
$conditionMember = 'select m.realname from ' . tablename('bj_qmxk_member') . ' m where m.from_user=o.from_user and m.weid=' . $_W['weid'];
$pindex = max(1, intval($_GPC['page']));
$psize = 30;
$list = pdo_fetchall("SELECT o.*,'' as commissions,1 as level,( {$conditionMember}) realname FROM " . tablename('bj_qmxk_order') . " o WHERE o.weid = '{$_W['weid']}' {$condition1}  union all (SELECT o.*,'' as commissions,2 as level,( {$conditionMember}) realname FROM " . tablename('bj_qmxk_order') . " o WHERE o.weid = '{$_W['weid']}' {$condition2} ) union all (SELECT o.*,'' as commissions,3 as level,( {$conditionMember}) realname FROM " . tablename('bj_qmxk_order') . " o WHERE o.weid = '{$_W['weid']}' {$condition3} )  ORDER BY  createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
$listx = pdo_fetchall("SELECT o.*,'' as commissions,1 as level,( {$conditionMember}) realname FROM " . tablename('bj_qmxk_order') . " o WHERE o.weid = '{$_W['weid']}' {$condition1}  union all (SELECT o.*,'' as commissions,2 as level,( {$conditionMember}) realname FROM " . tablename('bj_qmxk_order') . " o WHERE o.weid = '{$_W['weid']}' {$condition2} ) union all (SELECT o.*,'' as commissions,3 as level,( {$conditionMember}) realname FROM " . tablename('bj_qmxk_order') . " o WHERE o.weid = '{$_W['weid']}' {$condition3} )  ");
$total = sizeof($listx);
...
if (!empty($list)) {
    foreach ($list as $lkey => $l) {
        $commissions = pdo_fetchall('select *,\'\' as title,\'\' as thumb from ' . tablename('bj_qmxk_order_goods') . ' where orderid = ' . $l['id']);
        foreach ($commissions as $key => $commission) {
            $goods = pdo_fetch('select title,thumb from ' . tablename('bj_qmxk_goods') . ' where id = ' . $commission['goodsid']);
